...
-/
import HumanProof.RefinedDiscrTree.RefinedDiscrTree
...
open HumanProof RefinedDiscrTree Lean Meta
...
def getHypothesisRewrites (e : Expr) (except : Option FVarId) :
  ...
  let (results, _) ← getMatch (← getHypotheses except) e (unify := false) (matchRootStar := true)
...
def tacticString (rw : Rewrite) (occ : Option Nat) (loc : Option Name) (isLemma : Bool)
  ...
  let proof ← printRewriteLemma rw.proof false --isLemma
...
private def rpc (props : SelectInsertParams) : RequestM (RequestTask Html) :=
  ...
  goal.ctx.val.runMetaM {} do
    ...
    Meta.withLCtx lctx md.localInstances do
      ...
      let fvarId ← location.mapM ((·.fvarId) <$> Lean.Meta.getLocalDeclFromUserName ·)
